   ...
   const solve=()=>{
    ...
    for(let j=1;j<i;j++)
    {
        setTimeout(()=>{
            ...
            document.getElementById(ids[j]).childNodes[1].style.backgroundColor='blue';
            document.getElementById(ids[j-1]).childNodes[1].style.backgroundColor='blue';

            setTimeout(()=> {

              document.getElementById(ids[j]).childNodes[1].style.backgroundColor=myState.colors;
              document.getElementById(ids[j-1]).childNodes[1].style.backgroundColor=myState.colors;
    ...
    setTimeout(()=> {
        dispatch(updateValue({type:'PLAY_PAUSE',_play:false}));
        console.log('Bubble sort');
        dispatch(updateValue({type:'UPDATE_COLOR',color:'rgb(0,182,0)'}))
